...
if __name__ == '__main__':
    ...
    navmesh_tight_path = scene.replica_folder / scene_name / 'navmesh_tight.ply'
    ...
    for seq_id in range(seq_num):
        target_interaction_path = 'data/replica/room_0/goals/sit_stool_39.pkl'
        action = 'sit'
        obj_category = 'stool'
        obj_id = 39
        ...
        command = "python synthesize/gen_locomotion_unify.py --goal_thresh 0.5 --goal_thresh_final 0.2 --max_depth 120 --num_gen1 128 --num_gen2 16 --num_expand 8 " \
                  ...
                  "--visualize 0 --use_zero_pose 1 --use_zero_shape 1".format(seq_id, scene_path, scene_name,
                                                                              navmesh_tight_path, floor_height,
                                                                              wpath_path, path_name)
        print(command)
        os.system(command)
        ...
        command = "python synthesize/gen_interaction_unify.py --goal_thresh_final -1 --max_depth 15 --num_gen1 128 --num_gen2 32 --num_expand 4 " \
                  ...
                  "--visualize 0".format(seq_id, scene_path, scene_name, sdf_path, mesh_path, floor_height,
                                         target_body_path, interaction_name + '_down', last_motion_path)
        print(command)
        os.system(command)
