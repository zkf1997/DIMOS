...
if __name__ == "__main__":
    ...
    if load_goal_interaction:
        ...
        print(command)
        os.system(command)
        interaction_path_dir = Path('results/coins/two_stage/test/optimization_after_get_body') / action_in / f'{action_in}_{obj_name}_0/'
    ...
    for seq_id in range(num_seq):
        ...
        command = "python synthesize/gen_interaction_unify.py --goal_thresh_final -1 --max_depth 15 --num_gen1 128 --num_gen2 32 --num_expand 4 " \
                  "--project_dir . --cfg_policy ../results/exp_GAMMAPrimitive/MPVAEPolicy_babel_marker/sit_1frame " \
                  ...
                  "--visualize 0".format(seq_id, scene_path, scene_name, sdf_path, mesh_path, floor_height, target_body_path,
                                         seq_name, target_point_path)
        print(command)
        os.system(command)

        last_motion_path = f'results/interaction/{scene_name}/{seq_name}/MPVAEPolicy_babel_marker/sit_1frame/policy_search/seq000/results_ssm2_67_condi_marker_inter_0.pkl'
        """stand up"""
        command = "python synthesize/gen_interaction_unify.py --goal_thresh_final 0.3 --max_depth 10 --num_gen1 128 --num_gen2 16 --num_expand 8 " \
                  ...
                  "--use_zero_pose 0 --weight_target_dist 1 " \
                  "--visualize 0".format(seq_id, scene_path, scene_name, sdf_path, mesh_path, floor_height, target_point_path, interaction_name + '_up_' + str(seq_id), last_motion_path)
        print(command)
        os.system(command)