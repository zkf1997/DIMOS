...
if __name__ == '__main__':
    ...
    for seq_id in range(seq_num):
        ...
        """stand up"""
        command = "python synthesize/gen_interaction_unify.py --goal_thresh_final 0.2 --max_depth 10 --num_gen1 128 --num_gen2 32 --num_expand 4 " \
        ...
        command = (
            ...
            f"--gen_name policy_search --num_sequence 1 "
            f"--random_seed {seq_id} --scene_path {scene_path} --scene_name {scene_name} --navmesh_path {navmesh_tight_path} "
            f"--floor_height {floor_height:.2f} --wpath_path {wpath_path} --path_name {path_name} --last_motion_path {last_motion_path} "
            f"--clip_far 1 --history_mode 1 --weight_pene 1 "
            f"--visualize 0 --use_zero_pose 1 --use_zero_shape 1"
        )
        print(command)
