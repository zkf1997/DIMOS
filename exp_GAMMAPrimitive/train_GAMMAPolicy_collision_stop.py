...
def get_rewards(bparams, vertices, joints, Y_l, R0, T0, wpath, scene, points_local, local_map, last_step=False):
    ...
    terminate_goal = True if use_early_stop and (dist2target < cfg.trainconfig.goal_thresh).any() else False
    terminate_pene = True if cfg.trainconfig.use_pene_stop and (num_pene > cfg.trainconfig.pene_thresh).any() else False
    terminate = terminate_goal or terminate_pene
...
def rollout(body_s, max_depth=10, epoch=0, env_id=0):
...
def calc_loss_policy_val(traj_states,
    ...
    clipped_ratio = policy_ratio.clamp(1 - PPO_CLIP_VAL,
                                       1 + PPO_CLIP_VAL)
    traj_gae = (traj_gae - traj_gae.mean()) / (traj_gae.std() + 1e-10)
    if torch.isnan(traj_gae).any() or torch.isinf(traj_gae).any():
        print('traj gae error!', traj_gae)
...
if __name__ == '__main__':
    ...
    dataset_path = ''
    scene_dir = 'data/scenes'
    scene_list_path = Path('data/scenes/random_scene_0.4_names.pkl')
    with open(scene_list_path, 'rb') as f:
        scene_list = pickle.load(f)
    print('#scene:', len(scene_list))
    batch_gen = BatchGeneratorSceneTrain(dataset_path='',
    ...
    for epoch in tqdm(range(last_epoch, num_epochs)):
        ...
        for env_id in range(num_envs_per_epoch):
            ...
            with torch.no_grad():
                traj_ppo = rollout(data, max_depth=max_depth, epoch=epoch, env_id=env_id)
                if traj_ppo is None:
                    continue
        ...
        print('#1 frame samples:', N_samples)
        ...
        print('#2 frame samples:', N_samples)
