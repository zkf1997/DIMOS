...
from exp_GAMMAPrimitive.utils.batch_gen_amass import BatchGeneratorSitting
...
def get_feature(Y_l, pel, R0, T0, pt_wpath, scene):
    ...
    # # import trimesh
    # # p1 = trimesh.points.PointCloud(Y_l[0, 0].cpu().numpy(), colors=np.array([255, 0, 0, 255]))
    # # p2 = trimesh.points.PointCloud(obj_points_l[0, 0].cpu().numpy(), colors=np.array([0, 255, 0, 255]))
    # # (p1 + p2).show()
    # # use sdf to calc dists
    # Y_w = torch.einsum('bij,btpj->btpi', R0, Y_l) + T0[:, None, :, :]  # [b, t, p, 3]
    # sdf_values = calc_sdf(Y_w.reshape(nb * nt, -1, 3), scene['obj_sdf']).reshape(nb, nt, -1)
    # # p1 = trimesh.points.PointCloud(Y_w[0, 0].cpu().numpy(), colors=np.array([255, 0, 0, 255]))
    # # p2 = trimesh.points.PointCloud(obj_points.cpu().numpy(), colors=np.array([0, 255, 0, 255]))
    # # (p1 + p2).show()
    # print(scene['obj_sdf']['extent'] * 0.5)
    # sdf_values = sdf_values * scene['obj_sdf']['extent'] * 0.5
    # print(fea_marker_dist[0, 0])
    # print(sdf_values[0, 0])
    # print((torch.sqrt(fea_marker_dist) - sdf_values).max())

    return dist_xy, dist_xyz, fea_wpath, fea_marker_3d_n, fea_marker_h, target_ori_local, fea_marker_dist
...
def get_rewards(bparams, vertices, joints, Y_l, R0, T0, wpath, scene, target_ori_local, last_step=False):
    ...
    nb, nt = Y_l.shape[:2]
    h = 1/40
    Y_l_speed = torch.norm(Y_l[:, 2:, :, :2] - Y_l[:, :-2, :, :2], dim=-1) / (2 * h)
    ...
    dist2gp = torch.abs(Y_w[:, :, :, 2].amin(dim=-1)).mean(dim=-1)  # [b, t], feet on floor
    # dist2skat = Y_l_speed.amin(dim=-1)
    dist2skat = Y_l_speed.amin(dim=-1)
    r_contact_feet = torch.exp(-dist2gp)+torch.exp(-dist2skat)
    ...
    if args.profile:
        print('contact reward time', time.time() - t1)
...
def gen_tree_roots(data_mp, wpath, scene):
    ...
    distxy, dist, fea_wpath, fea_marker, fea_marker_h, target_ori_local, fea_marker_dist = get_feature(marker_seed, pelvis_loc[:,:t_his], R0, T0, wpath[-1:], scene)
    ...
    #     return None
    states = torch.cat([marker_seed,fea_marker, fea_marker_dist],dim=-1) if 'dist' in cfg_policy.modelconfig['body_repr'] else torch.cat([marker_seed,fea_marker],dim=-1)
    obj_points_local = None
    if 'pointcloud' in cfg_policy.modelconfig['body_repr']:
        obj_points_local = torch.einsum('bij,btj->bti', R0.permute(0,2,1), scene['obj_points'][None,...]-T0)  #  [b, p, 3]
    ...
    pred_vertices = smplxparser_mp.forward_smplx(betas=prev_betas,
                                        ...
                                        to_numpy=False).reshape([nb,nt,-1,3])
    traj_rewards = get_rewards(pred_params, pred_vertices, pred_joints, pred_marker_b, R0, T0, wpath, scene, target_ori_local,)
...
def expand_tree(data_mp, wpath, scene, last_step=False):
    ...
    distxy, dist, fea_wpath, fea_marker, fea_marker_h, target_ori_local, fea_marker_dist = get_feature(marker_seed, pel_loc_seed, R0, T0, wpath[-1:], scene)
...
def calc_loss_policy_val(traj_states,
    ...
    z_mu, z_logvar, value = policy_model(traj_states.permute([1,0,2]), obj_points=traj_obj, target_ori=traj_target_ori)
...
@hydra.main(version_base=None, config_path="../exp_GAMMAPrimitive/cfg", config_name="MPVAEPolicy_babel_sitting")
...
if __name__ == '__main__':
    ...
    # dataset_path = 'data/interaction.pkl'
    dataset_path = 'data/lie_sofa.pkl'
    batch_gen = BatchGeneratorSitting(dataset_path=dataset_path,
    ...
    # data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, visualize=False)
    # data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, visualize=False)
    data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, visualize=True, interaction_id=0)
    # exit()
    last_epoch = epoch
