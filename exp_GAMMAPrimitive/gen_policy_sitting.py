...
def expand_tree(mp_heap_prev, wpath, max_depth=10, scene=None):
    ...
    for iop in range(0, max_depth):
        ...
        if mp_heap_curr.len() == 0:
            ...
            # sys.exit()
        log_and_print(
            '[INFO] |--valid MPs={}, dist_to_target={:.2f}, path_finished={}/{}, dist_to_target_curr={:.2f}, dist_to_gp={:.2f}, dist_to_skat={:.2f}, pene_value={:.4f}'.format(
                mp_heap_curr.len(),
...
def gen_motion(body_s, max_depth=10):
    ...
    if not args.load_search or not os.path.exists(os.path.join(outfoldername, 'search')):
        ...
        if mp_heap_prev is None:
            return
        output = {'motion': None, 'wpath': wpath, 'markers': body_s['markers'],
...
if __name__ == '__main__':
    ...
    """get cfg"""
    hydra.initialize(version_base=None, config_path="../exp_GAMMAPrimitive/cfg")
    cfg_policy = hydra.compose(config_name=args.cfg_policy)
    ...
    batch_gen = BatchGeneratorSitting(dataset_path='data/interaction.pkl',
                                      ...
                                      body_model_path=bm_path)
    data = batch_gen.next_body(use_zero_pose=False, visualize=False)
    # batch_gen = BatchGeneratorSittingTest(dataset_path='data',
    #                                   body_model_path=bm_path)
    # from synthesize.get_scene import ReplicaScene
    # replica_folder = Path('/mnt/atlas_root/vlg-nfs/kaizhao/datasets/replica')
    # scene = ReplicaScene('room_0', replica_folder, build=False)
    # with open(replica_folder / 'room_0' / 'waypoints' / 'pelvis.json', 'r') as f:
    #     pelvis_frame = np.array(json.load(f)['pelvis_frame'])
    # mesh_path = replica_folder / 'room_0/instances/' / '74.ply'
    # sdf_path = Path('data', 'replica_sdf', 'room_0', '74.pkl')
    # data = batch_gen.next_body(use_zero_pose=False, visualize=True,
    #                            mesh_path=mesh_path, sdf_path=sdf_path, target_pelvis=pelvis_frame,
