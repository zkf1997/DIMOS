...
from exp_GAMMAPrimitive.utils.environments import BatchGeneratorInteraction2frameTrain
...
def canonicalize_2frame(data):
    ...
    ## get the last body param and marker in the new coordinate
    body_param_seed = smplxparser_2frame.update_transl_glorot(R0.repeat(t_his, 1, 1), T0.repeat(t_his, 1, 1),
                                                              betas=prev_betas,
                                                              gender=gender,
                                                              xb=prev_params.reshape(nb * nt, -1),
                                                              to_numpy=False,
...
def gen_tree_roots(data_mp, wpath, scene):
    ...
    motion_model = genop_2frame_male if gender == 'male' else genop_2frame_female
    """retrieve current states"""

    marker_seed = smplxparser_2frame.get_markers(
        betas=prev_betas,
        gender=gender,
    ...
    pelvis_loc = smplxparser_2frame.get_jts(betas=prev_betas,
                                            gender=gender,
                                            xb=body_param_seed.reshape(nb * nt, -1),
                                            to_numpy=False
                                            )[:, 0]  # [b*t, 3]
    ...
    return [[traj_states, traj_obj, traj_target_ori, traj_act, traj_act_logprob, traj_rewards, traj_value],
            [pred_marker_b, pred_params, prev_betas, gender, R0, T0, pred_pelvis_loc, '2-frame']]
...
def rollout(body_s, max_depth=10, epoch=0):
    ...
    data_mp0 = canonicalize_2frame(body_s)
