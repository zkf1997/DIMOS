...
if __name__=='__main__':
    ...
    if 'vlg-atlas' in hostname:
        base_folder = Path("/vlg-nfs/kaizhao/datasets/amass")
    elif 'dalcowks' in hostname:
        base_folder = Path("/home/kaizhao/dataset/amass/")
    else:
    ...
    print(get_seq_files('sit')[:4])
    print(get_seq_files('walk')[:4])
    print(get_seq_files('lie')[:4])
    exit()
    ...
    if N_MPS > 1:
        result_smplx_path = base_folder / 'gamma_turn/Canonicalized-MPx{:d}/data'.format(N_MPS)
    else:
        result_smplx_path = base_folder / 'gamma_turn/Canonicalized-MP/data'
    subsets = [
                # 'sit', 'lie',
               # 'walk',
               'turn'
