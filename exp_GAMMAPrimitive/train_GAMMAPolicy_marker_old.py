...
def get_feature(Y_l, pel, R0, T0, pt_wpath, scene):
    ...
    target_ori_local = target_ori_local[:, :2] / torch.norm(target_ori_local[:, :2], dim=-1,
                                                            keepdim=True).clip(
        min=1e-12)  # [b, 2], only care about xy projection on floor
    ...
    fea_marker_dist = chamfer_dists(Y_l.reshape(nb * nt, -1, 3), obj_points_l.reshape(nb * nt, -1, 3)).reshape(nb, nt,
                                                                                                               -1)  # b, t, 67
    # # import trimesh
    ...
    return dist_xy, dist_m_3d, fea_wpath, fea_marker_3d_n, fea_marker_h, target_ori_local, fea_marker_dist
...
def get_rewards(bparams, vertices, joints, Y_l, R0, T0, wpath, scene, target_ori_local, last_step=False):
    ...
    dist2gp = torch.abs(Y_w[:, :, :, 2].amin(dim=-1)).mean(dim=-1)  # feet on floor
    ...
    """dist to target"""
    dist2target = torch.norm(pt_markers_l_3d[:, -1, :, :] - Y_l[:, -1, :, :], dim=-1, keepdim=False).mean(dim=-1)  # [b]
...
def gen_tree_roots(data_mp, wpath, scene):
    ...
    distxy, dist, fea_wpath, fea_marker, fea_marker_h, target_ori_local, fea_marker_dist = get_feature(marker_seed,
                                                                                                       pelvis_loc[:,
                                                                                                       :t_his], R0, T0,
                                                                                                       wpath[-1:],
                                                                                                       scene)
    ...
    #     return None
    states = torch.cat([marker_seed, fea_marker, fea_marker_dist], dim=-1) if 'dist' in cfg_policy.modelconfig[
        'body_repr'] else torch.cat([marker_seed, fea_marker], dim=-1)
...
def expand_tree(data_mp, wpath, scene, last_step=False):
    ...
    distxy, dist, fea_wpath, fea_marker, fea_marker_h, target_ori_local, fea_marker_dist = get_feature(marker_seed,
                                                                                                       ...
                                                                                                       T0, wpath[-1:],
                                                                                                       scene)
    dist2target = dist[:, -1, :, 0].mean(dim=-1)
    # if USE_EARLY_STOP:
...
if __name__ == '__main__':
    ...
    # # data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, visualize=False, interaction_id=373)
    ...
    batch_gen = BatchGeneratorInteractionTrain(dataset_path='',
                                               ...
                                               body_model_path=bm_path)
    data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, visualize=True, reverse=False, interaction_id=35)
    ...
    for epoch in tqdm(range(last_epoch, num_epochs)):
        ...
        for _ in range(num_envs_per_epoch):
            data = batch_gen.next_body(sigma=GOAL_SIGMA, use_zero_pose=False, reverse=False)
            with torch.no_grad():
